...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchPastEvents = async () => {
      try {
        const response = await axios.get('http://localhost:3000/api/programclub/past-event');
  ...
  const handleAddEvent = async () => {
    try {
      ...
      await axios.post('http://localhost:3000/api/programclub/add-event', {
        eventName,
        eventDate,
        eventLink,
  ...
  const handleAddResult = async () => {
    try {
      await axios.post('http://localhost:3000/api/programclub/add-result', {
